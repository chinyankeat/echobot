    ...
    try{
        request(options, function (error, response, body) {
            if (process.env.DEVELOPMENT) {
                console.log("DB Log:" + body);
            }
        })
    } catch (e) {
        if (process.env.DEVELOPMENT) {
            console.log("cannot log to DB");
        }
    }
}
...
console.log("otp is " + session.userData.oneTimeCode);
        ...
        builder.Prompts.text(session, "I have just sent the One Time Code to you. Can you please key in the 4 digit code?", { listStyle:builder.ListStyle.button, maxRetries:MaxRetries, retryPrompt:DefaultErrorPrompt});
    },
    function (session, results) {
